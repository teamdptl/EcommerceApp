...
//                               @Param("rating") Integer rating, Pageable pageable);
  ...
  @Query("SELECT p as product, count(DISTINCT o.id) as orders, avg(r.rate) as rating, count(DISTINCT r.reviewId) as reviewer FROM Product p LEFT JOIN Review r ON r.product.productId = p.productId LEFT JOIN OrderLine o ON p.productId = o.product.productId WHERE p.isDeleted = false and (:title = '' or (:title != '' and p.name like %:title%)) " +
